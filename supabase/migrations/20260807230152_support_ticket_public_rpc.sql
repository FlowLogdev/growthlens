...
comment on function public.create_support_ticket(text, text, text, text)
  is 'Creates one validated public support ticket and returns only its generated receipt.';

create or replace function public.mark_support_ticket_email_delivery(
  p_ticket_id uuid,
...
begin
  ...
  set
    ...
    email_provider_id = case
      when p_delivery_status = 'sent' then left(p_provider_id, 500)
      else null
    end,
    updated_at = now()
...
revoke all on function public.mark_support_ticket_email_delivery(uuid, text, text) from public;
grant execute on function public.mark_support_ticket_email_delivery(uuid, text, text)
  to anon, authenticated, service_role;

comment on function public.mark_support_ticket_email_delivery(uuid, text, text)
  is 'Records the email delivery result for an unguessable support ticket UUID.';
